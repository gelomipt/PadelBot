...
async def register_for_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for game in games:
            ...
            button_text = f"{event_date_str} at {start_time_str} at {venue} for {capacity}"
...
async def handle_game_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    #    data = query.data  # e.g., 'select_game_123'
        ...
        cursor.execute("INSERT INTO registrations (game_id, player_id) VALUES (%s, %s)", (game_id, player_id))
        conn.commit()
        ...
        logger.info(f"User {telegram_user_id} registered for game {game_id}")
...
async def list_unconfirmed_registrations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
        SELECT r.id, s.event_date, s.start_time, s.venue, r.waiting
    ...
    for reg in registrations:
        ...
        if waiting_list:
            ...
            await update.message.reply_text("You still in waiting list for this game. Need to be in main list for confirm.") 
            cursor.close()
            conn.close()
        else:
            button_text = f"{event_date} at {start_time} - {venue}"
...
async def handle_confirm_registration_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_chat.send_message("Your registration has been confirmed.")
        ...
        game = cursor.fetchone()
        await send_game_status_update(game, context)
...
async def view_registrations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
        SELECT s.event_date, s.start_time, s.venue, r.is_confirmed, r.waiting
        FROM registrations r
        JOIN schedule s ON r.game_id = s.id
        WHERE r.player_id = %s
    """, (player_id,))
    ...
    for reg in registrations:
        ...
        venue = reg['venue']
        confirmed = "Confirmed" if reg['is_confirmed'] else "Unconfirmed"
